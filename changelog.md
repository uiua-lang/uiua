...
- Chage [`dip` `⊙`](https://uiua.org/docs/dip)'s glyph to reflect its relationship with [`gap` `∘`](https://uiua.org/docs/gap) and [`noop` `·`](https://uiua.org/docs/noop). Code using `→` with continue to work and will be formatted as `⊙`.
