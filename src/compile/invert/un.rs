...
pub static ANTI_PATTERNS: &[&dyn InvertPattern] = &[
    ...
    &(AntiBase, Base),
    &(Pop, Pop),
    &NoUnder((Identity, MatchPattern)),
    &(EncodeBytes, DecodeBytes(None)),
    &(DecodeBytes(None), EncodeBytes),
    &(Compress, AntiCompress),
    &(AntiCompress, Compress),
    &(
...
inverse!(PrimPat, input, _, Prim(prim, span), {
    let inv = match prim {
        ...
        Tan => ImplPrim(ATan, span),
        SinH => ImplPrim(ASinH, span),
        CosH => ImplPrim(ACosH, span),
        TanH => ImplPrim(ATanH, span),
        ...
        Reshape => ImplPrim(UnReshape, span),
        ...
        Oklch => ImplPrim(UnOklch, span),
        Json => ImplPrim(UnJson, span),
        Binary => ImplPrim(UnBinary, span),
        Compress => ImplPrim(UnCompress, span),
...
inverse!(ImplPrimPat, input, _, ImplPrim(prim, span), {
    let inv = match prim {
        ...
        ACos => Prim(Cos, span),
        ATan => Prim(Tan, span),
        ASinH => Prim(SinH, span),
        ACosH => Prim(CosH, span),
        ATanH => Prim(TanH, span),
        Ln => Prim(Exp, span),
        Exp2 => ImplPrim(Log2, span),
        ...
        UnMap => Prim(Map, span),
        UnStack => Prim(Args, span),
        UnReshape => ImplPrim(UnReshape, span),
        ...
        UnValidate => ImplPrim(ValidateImpl(None), span),
