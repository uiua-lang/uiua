...
impl SysBackend for NativeSys {
    ...
    fn ffi(
        ...
        let mut dyn_args: Vec<Box<dyn std::any::Any>> = Vec::new();
        for (i, (ty, arg)) in arg_tys.iter().zip(arg_values).enumerate() {
            macro_rules! scalar {
                ($arr:expr, $ty:ty) => {{
                    let val = $arr.data[0] as $ty;
                    dyn_args.push(Box::new(val));
                    args.push(Arg::new(dyn_args[i].downcast_ref::<$ty>().unwrap()));
                }};
            }
            match (ty, arg) {
        ...
        let cif = Cif::new(arg_tys.iter().map(cif_ty), cif_ty(&result_ty));
        ...
        Ok(unsafe {
            match result_ty {
                ...
                FfiType::Char => (cif.call::<c_schar>(fptr, &args) as f64).into(),
                FfiType::Short => (cif.call::<c_short>(fptr, &args) as f64).into(),
                FfiType::Int => (cif.call::<c_int>(fptr, &args) as f64).into(),
                FfiType::Long => (cif.call::<c_long>(fptr, &args) as f64).into(),
                FfiType::LongLong => (cif.call::<c_longlong>(fptr, &args) as f64).into(),
                FfiType::UChar => (cif.call::<c_uchar>(fptr, &args) as u8).into(),
                FfiType::UShort => (cif.call::<c_ushort>(fptr, &args) as f64).into(),
                FfiType::UInt => (cif.call::<c_uint>(fptr, &args) as f64).into(),
                FfiType::ULong => (cif.call::<c_ulong>(fptr, &args) as f64).into(),
                FfiType::ULongLong => (cif.call::<c_ulonglong>(fptr, &args) as f64).into(),
                FfiType::Float => (cif.call::<c_float>(fptr, &args) as f64).into(),
                FfiType::Double => (cif.call::<c_double>(fptr, &args) as f64).into(),
                FfiType::Ptr(inner) if *inner == FfiType::Char => {
                    let ptr = cif.call::<*const c_char>(fptr, &args);
                    let s = CStr::from_ptr(ptr).to_str().map_err(|e| e.to_string())?;
                    Value::from(s)
                }
                ty => return Err(format!("Invalid FFI return type {ty:?}")),
            }
        })
    }
}
