...
pub fn Design() -> impl IntoView {
    ...
    view! {
        ...
        <p>"Of course, I also made the discovery that juggling 3 or more values on the stack also imposes a high cognitive load on the developer. This is especially true if you try to "<em>"rotate"</em>" the stack like you can with "<Prim prim=Roll/>" and "<Prim prim=Unroll/>", which are currently deprecated. "<Prim prim=Fork/>" is based on the fork structure from other array languages, and it inspired my own invention, "<Prim prim=Trident/>". "<Prim prim=Dip/>" replaced the rolling functions as it is more general and easier to reason about."</p>
        <br/>
        <h3>"Expressions"</h3>
