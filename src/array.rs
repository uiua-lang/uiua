...
pub trait RealArrayValue: ArrayValue + Copy {
    /// Attempt to convert the value into a [`bool`]
    fn as_bool(self) -> Result<bool, AsBoolError>;
    /// Attempt to convert the value into a [`usize`]
    fn as_usize(self) -> Result<usize, AsIntError>;
    /// Attempt to convert the value into a [`isize`]
    fn as_isize(self) -> Result<isize, AsIntError>;
    /// Attempt to convert the value into a [`u64`]
    fn as_u64(self) -> Result<u64, AsIntError>;
    /// Attempt to convert the value into a [`u128`]
    fn as_u128(self) -> Result<u128, AsIntError>;
}
...
impl RealArrayValue for f64 {
    ...
    fn as_usize(self) -> Result<usize, AsIntError> {
        if self < 0.0 {
            return Err(AsIntError::Negative);
        }
        if self.fract().abs() > f64::EPSILON {
            return Err(AsIntError::NotInteger);
        }
        if self > usize::MAX as f64 {
            return Err(AsIntError::TooLarge);
        }
...
/// Errors for converting to an integer
...
/// Errors for converting to a boolean
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AsBoolError;

impl AsBoolError {
    pub(crate) fn format(self, initial: impl fmt::Display, value: impl RealArrayValue) -> String {
        format!("{initial}, but {value} is not a boolean")
    }
}
